...
REM Set paths
set RELEASE_DIR=..\..\src\Timelapse\bin\Publish\SelfContained-win-x64
set OUTPUT_DIR=..\bin\Release

REM Check if release folder exists
if not exist "%RELEASE_DIR%" (
    echo ERROR: Release folder not found: %RELEASE_DIR%
    echo Please publish Timelapse with SelfContained-win-x64 profile first.
    echo Run: dotnet publish ..\..\Timelapse\Timelapse.csproj -p:PublishProfile=SelfContained-win-x64
    pause
    exit /b 1
)
...
wix build Product.wxs Files.wxs ^
    ...
    -loc en-us.wxl ^
    -out "%OUTPUT_DIR%\TimelapseInstaller-PerUser.msi" ^
...
echo %CD%\%OUTPUT_DIR%\TimelapseInstaller-PerUser.msi
echo.

REM Clean up generated Files.wxs and .wixpdb
del Files.wxs
if exist "%OUTPUT_DIR%\TimelapseInstaller-PerUser.wixpdb" del "%OUTPUT_DIR%\TimelapseInstaller-PerUser.wixpdb"
