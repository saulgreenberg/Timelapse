...
namespace TimelapseTemplateEditor
{
    public partial class TemplateEditorWindow
    {
        private void DoCreateMetadataStandardFields(List<StandardsRow> metadataRowList, List<StandardsRow> imageTemplateRows, Dictionary<int, string> aliases)
        {
            ...
            List<ColumnTuplesWithWhere> infoColumnsTuplesWithWhereList = new List<ColumnTuplesWithWhere>(); // holds columns which have changed for the current control
            List<ColumnTuplesWithWhere> rowsColumnsTuplesWithWhereList = new List<ColumnTuplesWithWhere>(); // holds columns which have changed for the current control
            ...
            foreach (StandardsRow sr in imageTemplateRows)
            {
                ...
                rowsColumnsTuplesWithWhereList.Add(new ColumnTuplesWithWhere(columnTupleList, id++));
            }
        ...
        List<StandardsRow> GetStandardRowsFromCamtrapDPJson(JsonMetadataTemplate template, bool richTypes, List<string> usedDataLabels)
        {
            ...
            int uniqueNumber = 1;
            List<StandardsRow> rows = new List<StandardsRow>();
            foreach (field field in template.fields)
            {
                string type;
                string choices = null;
                if (null != field.constraints.@enum && field.constraints.@enum.Count > 0)
                {
                ...
                if (null != field.constraints.unique && true == field.constraints.unique)
                {
                ...
                StandardsRow row = new StandardsRow(type, 1, "", label, datalabel, tooltip, choices);
                rows.Add(row);
            }
            return rows;
        }
