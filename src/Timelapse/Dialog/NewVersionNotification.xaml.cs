...
namespace Timelapse.Dialog
{
    ...
    public partial class NewVersionNotification
    {
        ...
        public NewVersionNotification(Window owner, string applicationName, Version currentVersionNumber, Version latestVersionNumber)
        {
            ...
            // A patch is a release that only bumps Revision - Major.Minor.Build (the "real"
            // release) is unchanged - so it warrants gentler wording than a full new version.
            bool isPatchOnly = currentVersionNumber.Major == latestVersionNumber.Major
                               && currentVersionNumber.Minor == latestVersionNumber.Minor
                               && currentVersionNumber.Build == latestVersionNumber.Build;

            // Construct the template message
            Title = isPatchOnly
                ? $"An update to your current version of {applicationName} is available."
                : $"A new version of {applicationName} is available.";
            ...
            if (isPatchOnly)
            {
                Message.Reason = "This is a modest change (a patch) to the current release, usually to address a bug or issue.[br]" + Message.Reason;
            }
            ...
            // Create a flow document and load it with the contents of the file. Skipped for a
            // patch - the change notes file is keyed to full releases, not patch revisions, so
            // there's nothing meaningful to show here. Collapse the whole pane (separator lines
            // included), not just the FlowDocumentScrollViewer, and let the window shrink to fit
            // rather than leaving a blank gap where a fixed Height would otherwise still reserve
            // that pane's space.
            if (isPatchOnly)
            {
                ChangeDescriptionPane.Visibility = Visibility.Collapsed;
