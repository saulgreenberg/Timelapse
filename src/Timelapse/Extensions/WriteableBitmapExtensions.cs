...
namespace Timelapse.Extensions
{
    ...
    internal static class WriteableBitmapExtensions
    {
        ...
        extension(WriteableBitmap unaltered)
        {
            public unsafe WriteableBitmap CombinedDifference(WriteableBitmap previous, WriteableBitmap next, byte threshold)
            {
            ...
            public unsafe bool IsDark(int darkPixelThreshold, double darkPixelRatio, out double darkPixelFraction, out bool isColor)
            {
                // Check the arguments for null 
                ThrowIf.IsNullArgument(unaltered, nameof(unaltered));
                ...
                try
                {
                    ...
                    byte* currentPixel = (byte*)unaltered.BackBuffer.ToPointer(); // the imageIndex will point to a particular byte in the pixel array
                    int pixelSizeInBytes = unaltered.Format.BitsPerPixel / 8;
                    int pixelStride = ImageValues.DarkPixelSampleStrideDefault;
                    int totalPixels = unaltered.PixelHeight * unaltered.PixelWidth; // total number of pixels in the image
            ...
            public unsafe WriteableBitmap Subtract(WriteableBitmap image2)
            {
                // Check the arguments for null 
                ThrowIf.IsNullArgument(unaltered, nameof(unaltered));
                ThrowIf.IsNullArgument(image2, nameof(image2));

                if (BitmapsMismatched(unaltered, image2))
                {
                    return null;
                }
                GetColorOffsets(unaltered, out int blueOffset, out int greenOffset, out int redOffset);
                ...
                int totalPixels = unaltered.PixelWidth * unaltered.PixelHeight;
                int pixelSizeInBytes = unaltered.Format.BitsPerPixel / 8;
                byte* image1Index = (byte*)unaltered.BackBuffer.ToPointer();
