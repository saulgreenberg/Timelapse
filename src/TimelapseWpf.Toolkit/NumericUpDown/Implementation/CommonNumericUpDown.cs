...
{
  public abstract class CommonNumericUpDown<T>(
    ...
    where T : struct, IFormattable, IComparable<T>
  {
    ...
    private readonly FromText _fromText = fromText ?? throw new ArgumentNullException( nameof(fromText) );
    private readonly FromDecimal _fromDecimal = fromDecimal ?? throw new ArgumentNullException( nameof(fromDecimal) );
    private readonly Func<T, T, bool> _fromLowerThan = fromLowerThan ?? throw new ArgumentNullException( nameof(fromLowerThan) );
    private readonly Func<T, T, bool> _fromGreaterThan = fromGreaterThan ?? throw new ArgumentNullException( nameof(fromGreaterThan) );
    ...
    internal static readonly DependencyProperty IsInvalidProperty = DependencyProperty.Register( nameof(IsInvalid), typeof( bool ), typeof( CommonNumericUpDown<T> ), new UIPropertyMetadata( false ) );
    ...
    public static readonly DependencyProperty ParsingNumberStyleProperty =
        DependencyProperty.Register( nameof(ParsingNumberStyle), typeof( NumberStyles ), typeof( CommonNumericUpDown<T> ), new UIPropertyMetadata( NumberStyles.Any ) );
    ...
    private bool HandleNullSpin()
    {
      if( !Value.HasValue )
      {
        T forcedValue = DefaultValue ?? default( T );
    ...
    protected override T? ConvertTextToValue( string text )
    {
      ...
      var result = this.ConvertTextToValueCore( currentValueText, text );
    ...
    private T? ConvertTextToValueCore( string currentValueText, string text )
    {
      ...
      else
      {
        ...
        if( !_fromText( text, this.ParsingNumberStyle, CultureInfo, out var outputValue ) )
        {
          ...
          if( !_fromText( currentValueText, this.ParsingNumberStyle, CultureInfo, out _ ) )
          {
