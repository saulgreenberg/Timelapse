...
namespace TimelapseWpf.Toolkit.Primitives
{
  public class WindowContainer : Canvas
  {
    ...
    {
      this.SizeChanged += this.WindowContainer_SizeChanged;
      this.LayoutUpdated += this.WindowContainer_LayoutUpdated;
    ...
    protected override Size MeasureOverride( Size constraint )
    {
      ...
      if( this.Children.Count > 0 )
      {
        ...
        double height = double.IsNaN( this.Height )
                        ...
                        : this.Height;
        return new( Math.Min( width, constraint.Width), Math.Min( height, constraint.Height) );
      }
    ...
    protected override void OnVisualChildrenChanged( DependencyObject visualAdded, DependencyObject visualRemoved )
    {
      ...
      if( visualRemoved != null )
      {
        ...
        removedChild.LeftChanged -= this.Child_LeftChanged;
        removedChild.TopChanged -= this.Child_TopChanged;
        removedChild.PreviewMouseLeftButtonDown -= this.Child_PreviewMouseLeftButtonDown;
        removedChild.IsVisibleChanged -= this.Child_IsVisibleChanged;
        removedChild.IsKeyboardFocusWithinChanged -= this.Child_IsKeyboardFocusWithinChanged;
        if( removedChild is ChildWindow window )
        {
          window.IsModalChanged -= this.Child_IsModalChanged;
        }
      }
      ...
      if( visualAdded != null )
      {
        ...
        addedChild.LeftChanged += this.Child_LeftChanged;
        addedChild.TopChanged += this.Child_TopChanged;
        addedChild.PreviewMouseLeftButtonDown += this.Child_PreviewMouseLeftButtonDown;
        addedChild.IsVisibleChanged += this.Child_IsVisibleChanged;
        addedChild.IsKeyboardFocusWithinChanged += this.Child_IsKeyboardFocusWithinChanged;
        if( addedChild is ChildWindow window )
        {
          window.IsModalChanged += this.Child_IsModalChanged;
        }
      }
    ...
    {
      if (sender is not WindowControl windowControl)
        return;
    ...
    private void SetChildPos( WindowControl windowControl )
    {
      ...
      if( ( ( windowControl is MessageBox ) && ( windowControl.Left == 0 ) && ( windowControl.Top == 0 ) )
        || ( windowControl is ChildWindow { WindowStartupLocation: WindowStartupLocation.Center }) )
      {
      ...
      else
      {
        ...
        else
        {
          this.BringToFront( this.Children.OfType<WindowControl>()
                            .OrderByDescending( Canvas.GetZIndex )
    ...
    private void BringToFront( WindowControl windowControl )
    {
      if( windowControl != null )
      {
        int maxZIndez = this.Children.OfType<WindowControl>().Max( Canvas.GetZIndex );
    ...
    private bool IsModalWindow( WindowControl windowControl )
    {
      return ( ( ( windowControl is MessageBox ) && (windowControl.Visibility == Visibility.Visible) )
             || ( windowControl is ChildWindow { IsModal: true, WindowState: WindowState.Open }) );
    }
