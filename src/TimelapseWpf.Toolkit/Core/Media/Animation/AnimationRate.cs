...
namespace TimelapseWpf.Toolkit.Media.Animation
{
  ...
  public struct AnimationRate
  {
    ...
    public static AnimationRate Default { get; } = new( true );
    ...
    public override bool Equals( Object value )
    {
      ...
      if( value is AnimationRate rate )
    ...
    public bool Equals( AnimationRate animationRate )
    {
      ...
      else // HasSpeed
      {
        if( animationRate.HasSpeed )
        {
          ...
          return DoubleHelper.AreVirtuallyEqual( _speed, animationRate._speed );
        }
    ...
    public static AnimationRate operator +( AnimationRate t1, AnimationRate t2 )
    {
      if( t1.HasDuration && t2.HasDuration )
        return new( t1._duration + t2._duration );

      if( t1.HasSpeed && t2.HasSpeed )
        return new( t1._speed + t2._speed );
