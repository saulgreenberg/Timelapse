...
namespace Timelapse
{
    ...
    public partial class TimelapseWindow
    {
        ...
        {
            this.State.MostRecentImageSets.SetMostRecent(tdbDatabasePath);
            ...
            if (string.IsNullOrEmpty(fileDatabaseFilePath))
            {
                ...
                if (this.TrySelectDatabaseFile(tdbDatabasePath, out string selectedFileDatabaseFilePath, out importImagesAsNewDDBFile) == false)
                {
                    Mouse.OverrideCursor = null;
            ...
            if (false == Dialogs.DialogIsFileValid(this, fileDatabaseFilePath))
            {
                ...
                Mouse.OverrideCursor = null;
                return new Tuple<bool, string>(false, importImagesAsNewDDBFile ? fileDatabaseFilePath : string.Empty);
            }

            // Check: if Timelapse is in view-only state but needs to create a ddb file...
            if (this.State.IsViewOnly && importImagesAsNewDDBFile)
            {
            ...
            using (FileDatabase fileDB = await FileDatabase.UpgradeDatabasesAndCompareTemplates(fileDatabaseFilePath, this.templateDatabase, templateSyncResults)
                       .ConfigureAwait(true))
            {
                ...
                if (fileDB != null)
                {
                        ...
                        templateSyncResults.SyncRequiredAsFolderLevelsDiffer || templateSyncResults.ControlSynchronizationWarningsByLevel.Count > 0)
                    {
                        ...
                        Mouse.OverrideCursor = null;
                        TemplateChangedWizard templateChangedWizard = new TemplateChangedWizard(this, templateSyncResults,
                            this.templateDatabase.MetadataInfo, fileDB.MetadataInfo,
                            this.templateDatabase.MetadataControlsByLevel, fileDB.MetadataControlsByLevel,
                            this.templateDatabase.Controls, fileDB.Controls);
            ...
            FileDatabase fileDatabase = await FileDatabase
                .CreateOrOpenAsync(fileDatabaseFilePath, this.templateDatabase, this.State.CustomSelectionTermCombiningOperator, templateSyncResults, backUpJustMade)
                .ConfigureAwait(true);
            ...
            this.MetadataDataHandler = new MetadataDataEntryHandler(fileDatabase);
            ...
            this.CheckAndUpdateRootFolderIfNeeded(fileDatabase);
        ...
        private bool TryBeginImageFolderLoad(string imageSetFolderPath, string selectedFolderPath, bool isFirstTimeLoad)
        {
            ...
            List<string> foldersWithImagesOrVideos = new List<string>();
            ...
            int levels = this.templateDatabase.GetMetadataInfoTableMaxLevel();
            List<string> relativePathFolderList = new List<string>();
            if (levels > 0 && foldersWithImagesOrVideos.Count > 0)
            {
                ...
                List<Tuple<string, int>> divergentFolders = new List<Tuple<string, int>>(); // path, number of levels in path
                foreach (string path in foldersWithImagesOrVideos)
                {
                    ...
                    {
                        divergentFolders.Add(new Tuple<string, int>(path.Substring(rootPathLength), relativeImageFolderCount));
                    }
                    relativePathFolderList.Add(path.Substring(rootPathLength).TrimStart(Path.DirectorySeparatorChar));
                }
                ...
                if (divergentFolders.Count > 0)
                {
                    ...
                    Mouse.OverrideCursor = null;
                    MetadataFolderComplianceViewer dialog = new MetadataFolderComplianceViewer(this, this.DataHandler.FileDatabase, relativePathFolderList, this.DataHandler.FileDatabase.MetadataInfo);
            ...
            if (this.State.ImageMetadataAskOnLoad)
            {
                Cursor cursor = Mouse.OverrideCursor;
                PopulateFieldsWithImageMetadataOnLoad populateField = new PopulateFieldsWithImageMetadataOnLoad(this, this.DataHandler.FileDatabase, filesToAdd[0].FullName);
                if (this.ShowDialogAndCheckIfChangesWereMade(populateField))
                {
                    this.State.MetadataOnLoad = populateField.ImageMetadataOnLoad;
                }
                Mouse.OverrideCursor = cursor;
            }
        ...
        private async Task OnFolderLoadingCompleteAsync(bool filesJustAdded)
        {
            ...
            CustomSelection.SetDetectionRanges(this.DataHandler.FileDatabase.CustomSelection.DetectionSelections);
            ...
            this.DataHandler.FileDatabase.CreateFolderMetadataTablesIfNeeded();
            this.DataHandler.FileDatabase.MetadataTableLoadRowsFromDatabase();
            this.MetadataUI.InitalizeFolderMetadataTabs();
            this.MetadataUI.RelativePathToCurrentImage = null;
            this.MetadataUI.RelativePathToCurrentImage = this.DataHandler?.ImageCache?.Current?.RelativePath;
