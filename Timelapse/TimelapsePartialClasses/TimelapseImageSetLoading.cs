    ...
    public partial class TimelapseWindow
    {
        ...
        private bool TryBeginImageFolderLoad(string imageSetFolderPath, string selectedFolderPath, bool isFirstTimeLoad)
        {
            ...
            if (levels > 0 && foldersWithImagesOrVideos.Count > 0)
            {
                ...
                foreach (string path in foldersWithImagesOrVideos)
                {
                    ...
                    {
                        divergentFolders.Add(new Tuple<string, int>(path[rootPathLength..], relativeImageFolderCount));
                    }
                ...
                {
                    Mouse.OverrideCursor = null;
                    MetadataFolderComplianceViewer dialog = new MetadataFolderComplianceViewer(this, DataHandler.FileDatabase, relativePathFolderList, DataHandler.FileDatabase.MetadataInfo, true);
        ...
        private bool TrySelectDatabaseFile(string templateDatabasePath, out string databaseFilePath, out bool importImages)
        {
            ...
            else if (fileDatabasePaths.Length > 1)
            {
                ...
                ChooseFromListOfTimelapseFiles chooseDatabaseFile = new ChooseFromListOfTimelapseFiles(this, fileDatabasePaths, templateDatabasePath);
                Cursor cursor = Mouse.OverrideCursor;
                Mouse.OverrideCursor = null;
