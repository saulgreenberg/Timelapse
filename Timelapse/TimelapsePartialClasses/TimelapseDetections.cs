        ...
        public BoundingBoxes GetBoundingBoxesForCurrentFile(long fileID, bool initialFrameOnly)
        {
            ...
            if (DataHandler.FileDatabase.DetectionsExists())
            {
                ...
                foreach (DataRow detectionRow in dataRows)
                {
                    ...
                    BoundingBox box = new BoundingBox((string)detectionRow[DetectionColumns.BBox], confidence, frameNumber, (string)detectionRow[DetectionColumns.Category],
        ...
        public async Task<BoundingBoxes> GetBoundingBoxesForCurrentFileAsync(long fileID)
        {
            ...
            if (DataHandler.FileDatabase.DetectionsExists())
            {
                ...
                foreach (DataRow detectionRow in dataRows)
                {
                    ...
                    BoundingBox box = new BoundingBox((string)detectionRow[DetectionColumns.BBox], confidence, frameNumber, (string)detectionRow[DetectionColumns.Category], detectionCategoryLabel, classifications);
                    bboxes.Boxes.Add(box);
                }
            }
            return bboxes;
