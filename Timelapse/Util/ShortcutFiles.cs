    ...
    public static class ShortcutFiles
    {
        ...
        private static bool DoCreateShortcut(string pathToShortcutFile, string targetPath)
        {
            try
            {
                ...
                if (!(wsh.CreateShortcut(
                        Path.Combine(pathToShortcutFile, $"{Path.GetFileName(targetPath)} files.lnk")) is IWshShortcut shortcut))
                {
                    return false;
                }
                shortcut.Arguments = "";
                ...
                shortcut.Description = $"Shortcut to Image folders at {targetPath}";
                shortcut.Save();
                return true;
            }
            catch
