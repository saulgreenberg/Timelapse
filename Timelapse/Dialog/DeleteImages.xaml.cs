    ...
    public partial class DeleteImages
    {
        ...
        private void DeleteCurrentImageOnly()
        {
            ...
            if (deleteImage)
            {
                if (this.backupDeletedFiles)
                {
                    ...
                    Message.Result = $"\u2022 The deleted {imageOrVideo} will be backed up in a sub-folder named {File.DeletedFilesFolder}.";
                }
                else
                {
                    Message.Title = $"Permanently delete the current {imageOrVideo}";
                    Message.What = $"Permanently delete the current {imageOrVideo} if it exists.";
                    Message.Result = $"\u2022 The deleted {imageOrVideo} will be permanently deleted.";
                }
                ...
                if (deleteImage && deleteData == false)
                {
                    ...
                    Message.What +=
                        $"{Environment.NewLine}The data associated with this {imageOrVideo} IS NOT deleted.";
                    Message.Result += $"{Environment.NewLine}\u2022 A placeholder {imageOrVideo} (and its data) will be shown when you try to view it.";
                    if (this.backupDeletedFiles)
                    {
                        Message.Hint += $"{Environment.NewLine}\u2022 You can restore the backed up deleted {imageOrVideo} by moving it from {File.DeletedFilesFolder} back to its original location.";
                    }
                }
                else if (deleteImage && deleteData)
                {
                    ...
                    Message.What +=
                        $"{Environment.NewLine}Permanently deletes the data associated with this {imageOrVideo}.";
                    Message.Result +=
                        $"{Environment.NewLine}\u2022 The data associated with this {imageOrVideo} IS PERMANENTLY DELETED.";
                }
            }
            else
            {
                ...
                Message.What = $"Deletes the data associated with the current {imageOrVideo}, but leaves the {imageOrVideo} intact";
                Message.Result = $"\u2022 This data record will be removed.{Environment.NewLine}";
                Message.Result +=
                    $"\u2022 The {imageOrVideo} is still intact, but it will not be displayed in Timelapse unless a duplicate record exists.";
                Message.Hint += $"{Environment.NewLine}\u2022 Deleting only the data is useful for removing a previously-created duplicate record of a file.";
            }
        }
        ...
        private void DeleteMultipleImages()
        {
            ...
            if (deleteImage)
            {
                if (this.backupDeletedFiles)
                {
                    Message.Title = $"Delete {numberOfImagesToDelete} files ";
                    Message.What = $"Delete {numberOfImagesToDelete} image and/or video(s) - if they exist - marked for deletion.";
                    Message.Result = $"\u2022 The deleted file will be backed up in a sub-folder named {File.DeletedFilesFolder}.";
                }
                else
                {
                ...
                if (deleteData == false)
                {
                    ...
                    Message.What += $"{Environment.NewLine}The data associated with these files IS NOT deleted.";
                    Message.Result += $"{Environment.NewLine}\u2022 A placeholder image (and its data) will be shown when you try to view a deleted file.";
                    if (this.backupDeletedFiles)
                    {
                        Message.Hint += $"{Environment.NewLine}\u2022 You can restore these deleted files by moving them from {File.DeletedFilesFolder} back to its original location.";
                    }
                }
                else // (deleteData == true)
                {
                    ...
                    Message.What += $"{Environment.NewLine}Permanently deletes the data associated with these files.";
                    Message.Result += $"{Environment.NewLine}\u2022 The data associated with these image or video files IS PERMANENTLY DELETED.";
            ...
            else
            {
                ...
                Message.Result +=
                    $"\u2022 The {numberOfImagesToDelete} file(s) are still intact, but will not be displayed in Timelapse unless a duplicate record exists.";
                Message.Hint += $"{Environment.NewLine}• Deleting only the data  is useful for removing previously-created duplicate records of a file.";
            }
        }
        #endregion
