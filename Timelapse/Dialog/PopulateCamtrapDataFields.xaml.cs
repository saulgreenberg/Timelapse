...
namespace Timelapse.Dialog
{
    ...
    public partial class PopulateCamtrapDataFields
    {
                ...
                while (imageIndex < TotalImages)
                {
                    ...
                    if (Episodes.Episodes.EpisodesDictionary.Count <= 1)
                    {
                        ...
                        observationLevel = eventBasedStrategy
                            ? "event"
                            : "media";
                        observationLevelColumnTuple = [new ColumnTuple(dataLabelObservationLevel, observationLevel)];
                        imagesToUpdate.Add(new ColumnTuplesWithWhere(observationLevelColumnTuple, ID));
                    ...
                    else
                    {
                        ...
                        if (eventBasedStrategy)
                        {
                            ...
                            observationLevelColumnTuple = [new ColumnTuple(dataLabelObservationLevel, "event")];
                            eventIDColumnTuple = [new ColumnTuple(dataLabelEventID, Guid.NewGuid().ToString())];
                            foreach (KeyValuePair<int, Tuple<int, int>> unused in Episodes.Episodes.EpisodesDictionary)
                            {
                                ...
                                imagesToUpdate.Add(new ColumnTuplesWithWhere(eventIDColumnTuple, ID));
                                imagesToUpdate.Add(new ColumnTuplesWithWhere(observationLevelColumnTuple, ID));
                        ...
                        else
                        {
                            ...
                            foreach (KeyValuePair<int, Tuple<int, int>> unused in Episodes.Episodes.EpisodesDictionary)
                            {
                                ...
                                imagesToUpdate.Add(new ColumnTuplesWithWhere(eventIDColumnTuple, fileDatabase.FileTable[imageIndex].ID));
                                imagesToUpdate.Add(new ColumnTuplesWithWhere(observationLevelColumnTuple, ID));
        ...
        private static void HelperPopulateFileNameAndRelativePath(List<ColumnTuplesWithWhere> imagesToUpdate, string relativePath, string fileName,  long ID)
        {
            ...
            List<ColumnTuple> fileNameColumnTuple = [new ColumnTuple(fileNameDataLabel, fileName)];
            imagesToUpdate.Add(new ColumnTuplesWithWhere(fileNameColumnTuple, ID));
            
            string relativePathWithFileName = System.IO.Path.Combine(relativePath, fileName);
            List<ColumnTuple> filePathColumnTuple = [new ColumnTuple(filePathDataLabel, relativePathWithFileName)];
            imagesToUpdate.Add(new ColumnTuplesWithWhere(filePathColumnTuple, ID));
        }
        ...
        private static void HelperPopulateStartEndEventDate(List<ColumnTuplesWithWhere> imagesToUpdate, DateTime startDate, DateTime endDate, long ID)
        {
            ...
            List<ColumnTuple> eventStartColumnTuple = [new ColumnTuple(dataLabelEventStart, startDate)];
            List<ColumnTuple> eventEndColumnTuple = [new ColumnTuple(dataLabelEventEnd, endDate)];
            imagesToUpdate.Add(new ColumnTuplesWithWhere(eventStartColumnTuple, ID));
            imagesToUpdate.Add(new ColumnTuplesWithWhere(eventEndColumnTuple, ID));
        }
        ...
        private static void HelperPopulateTimeStamp(List<ColumnTuplesWithWhere> imagesToUpdate, DateTime date, long ID)
        {
            ...
            List<ColumnTuple> columnTuple = [new ColumnTuple(dataLabel, date)];
            imagesToUpdate.Add(new ColumnTuplesWithWhere(columnTuple, ID));
        }

        private static void HelperPopulateDataLabelWithGuid(List<ColumnTuplesWithWhere> imagesToUpdate, string dataLabel, long ID)
        {
            List<ColumnTuple> columnTuple = [new ColumnTuple(dataLabel, Guid.NewGuid().ToString())];
            imagesToUpdate.Add(new ColumnTuplesWithWhere(columnTuple, ID));
        }
        ...
        {
            List<ColumnTuple> columnTuple = [new ColumnTuple(dataLabel, relativePath)];
            imagesToUpdate.Add(new ColumnTuplesWithWhere(columnTuple, ID));
        }
        #endregion
    }
