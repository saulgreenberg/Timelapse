...
namespace Timelapse.Dialog
{
    ...
    public partial class PopulateFieldWithEpisodeData
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            CheckBoxIncludeEpisodeID.IsChecked = IncludeAnEpisodeIDNumber;

            CheckBoxIncludeEpisodeID.Checked += CheckBoxIncludeEpisodeID_CheckChanged;
            CheckBoxIncludeEpisodeID.Unchecked += CheckBoxIncludeEpisodeID_CheckChanged;
            ...
            IncludeAnEpisodeIDNumber = CheckBoxIncludeEpisodeID.IsChecked == true;
                ...
                while (imageIndex < TotalImages)
                {
                    ...
                    if (Episodes.Episodes.EpisodesDictionary.Count <= 1)
                    {
                        ...
                        string singletonData =
                            $"{(IncludeAnEpisodeIDNumber ? EpisodeCount + ":" : string.Empty)}1|1";

                        List<ColumnTuple> ctl = new List<ColumnTuple> { new ColumnTuple(dataLabelByLabel[dataFieldLabel], singletonData) };
                        imagesToUpdate.Add(new ColumnTuplesWithWhere(ctl, fileDatabase.FileTable[imageIndex].ID));
                        SingleCount++;
                        imageIndex++;
                    }
                    else
                    {
                        ...
                        foreach (KeyValuePair<int, Tuple<int, int>> episode in Episodes.Episodes.EpisodesDictionary)
                        {
                            ...
                            {
                                new ColumnTuple(dataLabelByLabel[dataFieldLabel],
                                    $"{(IncludeAnEpisodeIDNumber ? EpisodeCount + ":" : string.Empty)}{episode.Value.Item1}|{episode.Value.Item2}")};
                            imagesToUpdate.Add(new ColumnTuplesWithWhere(ctl, fileDatabase.FileTable[imageIndex].ID));
                            imageIndex++;
                        }
                    }
        ...
        {
            if (true == CheckBoxIncludeEpisodeID.IsChecked)
            {
                TBEpisode.Text = "Episode:    23:1|7";
                TBSingleton.Text = "Singleton: 23:1|1";
            }
            else
            {
        ...
        private void CheckBoxIncludeEpisodeID_CheckChanged(object sender, RoutedEventArgs e)
        {
            ...
            {
                IncludeAnEpisodeIDNumber = cb.IsChecked == true;
                ShowExampleFormat();
            }
        }
    }
