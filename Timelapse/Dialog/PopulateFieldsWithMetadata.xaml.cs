...
namespace Timelapse.Dialog
{
    ...
    public partial class PopulateFieldsWithMetadata
    {
        ...
        private readonly Dictionary<string,string> DataLabelControlTypeDict = new Dictionary<string,string>();
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (this.useDateMetadataOnly)
            {
                ...
                this.Title = "Populate dates and times from a metadata field";
            ...
            {
                if (this.useDateMetadataOnly &&
                    (control.Type == Constant.DatabaseColumn.DateTime || control.Type == Constant.Control.DateTime_ || 
                     control.Type == Constant.Control.Date_ || control.Type == Constant.Control.Time_))
                {
                ...
                else if (false == this.useDateMetadataOnly &&
                         Util.IsCondition.IsControlType_Note_MultiLine(control.Type))
                {
                    ...
                    foreach (KeyValuePair<string, string> kvp in this.MetadataGrid.SelectedMetadata)
                    {
                        ...
                        if (this.useDateMetadataOnly)
                        {
                            ...
                            if (DateTimeHandler.TryParseMetadataDateTaken(metadataValue, out DateTime metadataDateTime))
                            {
                                ...
                                {
                                    // NO! USE IMAGE ROW
                                    dateTimeStringToUse = Util.DateTimeHandler.ToStringDatabaseDateTime(metadataDateTime);
                                ...
                                else
                                {
                                    ...
                                    if (controlType == Constant.Control.DateTime_)
                                    {
                                        dateTimeStringToUse = Util.DateTimeHandler.ToStringDatabaseDateTime(metadataDateTime);
                                    }
                                    else if (controlType == Constant.Control.Date_)
                                    {
                                        dateTimeStringToUse = Util.DateTimeHandler.ToStringDatabaseDate(metadataDateTime);
                                    }
                                    else if (controlType == Constant.Control.Time_)
                                    {
                                        dateTimeStringToUse = Util.DateTimeHandler.ToStringTime(metadataDateTime);
                                    }
                                    if (false == string.IsNullOrEmpty(dateTimeStringToUse) )
                                    {
                                        imageUpdate = new ColumnTuplesWithWhere(new List<ColumnTuple>() { new ColumnTuple(dataLabelToUpdate, dateTimeStringToUse) }, image.ID);
