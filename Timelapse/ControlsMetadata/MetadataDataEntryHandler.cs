...
{
    public class MetadataDataEntryHandler : IDisposable
    {
        ...
        private void SetContextMenuCallbacks(MetadataDataEntryControl control)
        {
            ...
            else if (control is MetadataDataEntryAlphaNumeric alphaNumeric)
            {
        ...
        protected virtual void Container_PreviewMouseRightButtonDown(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                if (control is MetadataDataEntryNote || control is MetadataDataEntryMultiLine)
                {
                    // Any string is valid
                ...
                else if (control is MetadataDataEntryDecimalAny)
                {
                    ...
                    menuItemPasteFromClipboard.IsEnabled = Double.TryParse(clipboardText, out _);
                    clipboardText = clipboardText.Trim('0');
                ...
                else if (control is MetadataDataEntryDecimalPositive)
                {
                    ...
                    menuItemPasteFromClipboard.IsEnabled = Double.TryParse(clipboardText, out double x) && x >= 0;
                    clipboardText = clipboardText.Trim('0');
                    if (string.IsNullOrEmpty(clipboardText) || clipboardText == ".")
                    {
                        clipboardText = "0";
                ...
                else if (control is MetadataDataEntryMultiChoice multiChoice)
                {
                    // Only a value present as a menu choice is valid 
                    menuItemPasteFromClipboard.IsEnabled = false;
                    CheckComboBox comboBox = multiChoice.ContentControl;
                    foreach (Object t in comboBox.Items)
                    {
                        // This check skips over the Separator
        ...
        {
            if (sender is ComboBox comboBox && comboBox.SelectedItem is ComboBoxItem cbi)
            {
        ...
        public void UpdateMetadataTableAndMetadataDatabase(MetadataDataEntryControl control)
        {
            ...
            if (control is MetadataDataEntryDateTimeCustom dateTimeCustom)
            {
                if (dateTimeCustom.ContentControl.Value != null)
                {
                    ...
                    columnToUpdate.Columns.Add(new ColumnTuple(control.DataLabel, DateTimeHandler.ToStringDatabaseDateTime(dateTime))); // Populate the datetime data using the database datetime format
                }
            }
            else if (control is MetadataDataEntryDate date)
            {
                if (date.ContentControl.Value != null)
                {
                    ...
                    columnToUpdate.Columns.Add(new ColumnTuple(control.DataLabel, DateTimeHandler.ToStringDatabaseDate(dateTime))); // Populate the datetime data using the database datetime format
                }
            }
            else
            {
                columnToUpdate.Columns.Add(new ColumnTuple(control.DataLabel, control.Content)); // Populate the data 
            }
