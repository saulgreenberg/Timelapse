...
namespace Timelapse.Controls
{
    ...
    public partial class FormattedMessageContent
    {
        ...
        private void AddPropertySection(string propertyName, string propertyValue)
        {
            ...
            var grid = new Grid
            {
                Margin = new Thickness(0, 0, 0, 15) // Bottom margin for spacing between sections
            };

            // Define two columns: 80px for heading, remaining space for content
            ...
            var headingLabel = new Label
            {
                ...
                Padding = new Thickness(0, -6, 10, 0) // Further increased negative top padding to move label text up for baseline alignment
            };
        ...
        private void AddDetailsSeparator()
        {
            ...
            {
                Margin = new Thickness(0, 10, 0, 10) // Vertical spacing around separator
            };
        ...
        private void AddDetailsSection()
        {
            ...
            var toggleArrow = new TextBlock
            {
                ...
                FontWeight = FontWeights.Bold,
                VerticalAlignment = VerticalAlignment.Top,
                Margin = new Thickness(0, 2, 5, 0) // Align with heading text and add right spacing
            };
            ...
            var toggleText = new TextBlock
            {
                ...
                VerticalAlignment = VerticalAlignment.Top,
                Margin = new Thickness(0, 2, 0, 0) // Align with heading text
            };
            ...
            contentContainer.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(75) });
            contentContainer.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
            contentContainer.Margin = new Thickness(0, 5, 0, 0);
        ...
        // Handles const fields, static readonly fields, and runtime-evaluated static properties (including interpolated strings)
        private string ResolveStaticReferences(string text)
        {
            ...
            return Regex.Replace(text, staticRefPattern, match =>
            {
                ...
                // Map namespace prefixes to actual namespaces
                var fullTypeName = namespacePrefix switch
                {
                    "constant" => $"Timelapse.Constant.{className}",
                    _ => $"Timelapse.{namespacePrefix}.{className}"
                };
                ...
                try
                {
                    ...
                    if (type != null)
                    {
                        // Try field first (for const and static fields, including interpolated strings)
                        ...
                        // Try property (for static properties with getters)
        ...
        private static void AddSingleBulletItem(List<Inline> inlines, string content, int level)
        {
            ...
            grid.Margin = new Thickness(totalIndent, 0, 0, 0);
            
            // Define columns: fixed width for level 2 bullets for tighter spacing, auto for others
            var bulletColumnWidth = level == 2 ? new GridLength(8) : GridLength.Auto; // Very tight spacing for level 2 bullets
            grid.ColumnDefinitions.Add(new ColumnDefinition { Width = bulletColumnWidth });
            grid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
            ...
            var bulletLabel = new Label
            {
                ...
                Margin = level == 2 ? new Thickness(0, 0, -4, 0) : new Thickness(0), // Increased negative right margin for level 2 very tight spacing
                VerticalAlignment = VerticalAlignment.Top,
                HorizontalAlignment = HorizontalAlignment.Left
            };
            ...
            var itemInlines = ParseInlineFormatting(content);
        ...
        private void AddSingleNumberedItem(List<Inline> inlines, string content, int level, Dictionary<int, int> globalNumbering)
        {
            ...
            {
                2 => new GridLength(14), // Very tight spacing for letters with parentheses: a) b) c)
                3 => new GridLength(20), // Very tight spacing for Roman numerals: i) ii) iii)  
                _ => GridLength.Auto     // Auto sizing for level 1: 1. 2. 3.
            };
            grid.ColumnDefinitions.Add(new ColumnDefinition { Width = numberColumnWidth });
            grid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
            ...
            var numberLabel = new Label
            {
                ...
                Margin = level switch
                {
                    2 => new Thickness(0, 0, -3, 0), // Increased negative right margin for level 2 very tight spacing
                    3 => new Thickness(0, 0, -2, 0), // Negative right margin for level 3 very tight spacing
                    _ => new Thickness(0)            // Default margin for level 1
                },
                VerticalAlignment = VerticalAlignment.Top,
                HorizontalAlignment = HorizontalAlignment.Left
            };
        ...
        private static List<Inline> ParseInlineFormatting(string text)
        {
            var inlines = new List<Inline>();
            ...
            foreach (var segment in filteredSegments)
            {
                ...
                var formattedInline = CreateFormattedInline(segment.type, segment.content, segment.extra);
        ...
        private static Inline CreateFormattedInline(string type, string content, string extra)
        {
            return type switch
            {
                "bold" => CreateSemiBoldInline(content),                      // [b]text[/b] -> Semi-bold with nested parsing
                "italic" => CreateNestedFormattedInline<Italic>(content),       // [i]text[/i] -> Italic with nested parsing
                "underline" => CreateNestedFormattedInline<Underline>(content), // [u]text[/u] -> Underlined with nested parsing
                "emphasis" => CreateEmphasisInline(content),                   // [e]text[/e] -> Semi-bold Italic with nested parsing
                "fontsize" => CreateFontSizeInline(content, extra),           // [f #]text[/f] -> Font size adjusted with nested parsing
                "color" => CreateColorInline(content, extra),                 // #Color[text] -> Colored with nested parsing
        ...
        private static T CreateNestedFormattedInline<T>(string content) where T : Span, new()
        {
            var element = new T();
            var nestedInlines = ParseInlineFormatting(content); // Recursive parsing
        ...
        private static Span CreateSemiBoldInline(string content)
        {
            var span = new Span
            {
        ...
        private static Span CreateEmphasisInline(string content)
        {
            var span = new Span
            {
                FontWeight = FontWeights.SemiBold
        ...
        private static Inline CreateFontSizeInline(string content, string sizeAdjustment)
        {
            ...
            try
            {
                ...
                if (int.TryParse(sizeAdjustment, out var adjustment))
                {
                    ...
                    // Use the default WPF font size as base (typically 12pt)
                    var currentSize = 12.0; // Default font size baseline
            ...
            {
                // If parsing fails, use default font size
            }
        ...
        private static Inline CreateColorInline(string content, string colorName)
        {
            var span = new Span();
        ...
        private static Inline CreateLineBreakWithSpacing(string spacingParam = "")
        {
