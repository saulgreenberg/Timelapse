...
namespace Timelapse.Controls
{
    ...
    public class TreeViewWithClassificationDescriptions : TreeView
    {
        ...
        public bool HasContent => null != this.TaxaItemList && this.TaxaItemList.Count > 0;
        #endregion
        ...
        {
            List<string> taxaStrings1 = classificationDescriptionsDictionary.Values.ToList();
            List<string> taxaStrings2 = new List<string>();
            foreach (string taxaString in taxaStrings1)
            {
                taxaStrings2.Add(taxaString.Substring(taxaString.IndexOf(';') + 1));
            }
            taxaStrings2.Sort();
            this.TaxaItemProvider = new TaxaItemProvider(taxaStrings2);
        ...
        private void CollapseAllRecursive(TreeViewItem treeViewTaxaItem)
        {
        ...
        private void UnselectAllRecursive(TreeViewItem treeViewTaxaItem)
        {
    ...
    {
        public List<TaxaItem> TaxaItems { get; set; } = new List<TaxaItem>();
    }
    ...
    internal class TaxaItemProvider
    {
        ...
        private List<TaxaItem> GetTaxaItems(List<string> paths, string pathToHere)
        {
            ...
            List<TaxaItem> TaxaItems = new List<TaxaItem>();
            foreach (ListTaxaItem listTaxaItem in listTaxaItems)
            {
                ...
                PathTaxaItem TaxaItem = new PathTaxaItem
                {
        ...
        {
            List<ListTaxaItem> splitLists = new List<ListTaxaItem>();
            ListTaxaItem commonRootPathList = new ListTaxaItem();
            ...
            foreach (string path in paths)
            {
                ...
                if (currentParent != parent && false == string.IsNullOrEmpty(parent))
                {
                    ...
                    commonRootPathList = new ListTaxaItem()
                    {
                        PathHead = parent,
